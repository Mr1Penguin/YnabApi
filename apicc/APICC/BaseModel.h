...
namespace apicc {
    struct BaseModel {
        ...
        template<class T>
        struct TVec { using value_type = T; };
        ...
        template<class T>
        struct is_ivector : std::false_type
        {};

        template<class T>
        struct is_ivector<winrt::Windows::Foundation::Collections::IVector<T>> : std::true_type
        {};

        template<class T>
        struct is_tvec_type : std::false_type
        {};

        template<class T>
        struct is_tvec_type<TVec<T>> : std::true_type
        {};

        template<class T>
        struct convert_vector
        {
            using new_type = nullptr_t;
        };

        template<class T>
        struct convert_vector<TVec<T>>
        {
            static constexpr bool is_derived = std::is_base_of_v<::apicc::BaseModel, TVec<T>::value_type>;
            using new_type = winrt::Windows::Foundation::Collections::IVector<std::conditional_t<
                is_derived,
                typename get_class_type<typename TVec<T>::value_type, is_derived>::type,
                typename TVec<T>::value_type>>;
        };

        template<class T>
        struct convert_vector<TVec<TVec<T>>>
        {
    ...
    protected:
        ...
        static void Write(const winrt::Windows::Foundation::Collections::IVector<T> & vec, rwriter & writer,
                          const wchar_t * key, rapidjson::SizeType key_length, bool forceNull = false) noexcept {
            ...
            for (T && v : vec) {
                Write<I>(v, writer,
                      static_cast<const wchar_t *>(nullptr), static_cast<rapidjson::SizeType>(0));
            }
            writer.EndArray();
        }

        template<class I, rapidjson::SizeType N>
        static void Write(const winrt::Windows::Foundation::Collections::IVector<typename I::class_type> & vec, rwriter & writer,
                          const wchar_t(&key)[N], bool forceNull = false) noexcept {
            Write<I>(vec, writer, static_cast<const wchar_t *>(key), N, forceNull);
        }
        ...
        static void Write(const winrt::Windows::Foundation::Collections::IVector<typename I::class_type> & vec, rwriter & writer,
            ...
            for (typename I::class_type && v : vec) {
                Write(winrt::get_self<I>(v), writer, 
                      static_cast<const wchar_t *>(nullptr), static_cast<rapidjson::SizeType>(0));
            }
            writer.EndArray();
        }
